 ...
 *
 * Two TestBed shapes on purpose (docs/EXEMPLARS.md §8): `getWorkorderPickList`,
 * `getPickedItems`, and `consumePickedItems` are asserted through
 * `HttpTestingController`, the one place that actually exercises `HttpClient`
 * end to end; the per-pick-task facade operations migrated off `/workexec/**`
 * (issue #369) are asserted by mocking `WorkorderPickFacadeService` directly,
 * the shape a new domain-service spec should use.
 */
...
describe('InventoryPickService', () => {
  describe('reads composed over HttpClient (getWorkorderPickList / getPickedItems / consumePickedItems)', () => {
    ...
    it('getPickedItems — GET /v1/workorders/{workorderId}/picked-items', () => {
    ...
    it('consumePickedItems — POST /v1/workorders/{workorderId}/picked-items:consume', () => {
  ...
  // issue #369: resolvePickScan/confirmPickLine/completePickTask used to post to
  // /workexec/v1/workorders/{id}/picks/* — a gateway prefix pos-api-gateway never
  // routed (404 on every scan/confirm/complete). Migrated to the per-pick-task
  // WorkorderPickFacadeService operations; asserted here against the mocked SDK
  // facade's full positional argument list (docs/EXEMPLARS.md §8), not HTTP.
  describe('per-pick-task facade operations (WorkorderPickFacadeService)', () => {
    let service: InventoryPickService;
    const facade = {
    ...
    const taskFixture: WorkorderPickTaskResponse = {
      locationId: 'bin-001',
      pickListId: 'pl-001',
      pickTaskId: 'task-001',
      pickedQty: 2,
    ...
    beforeEach(() => {
      ...
      TestBed.configureTestingModule({
        providers: [
          InventoryPickService,
          { provide: WorkorderPickFacadeService, useValue: facade },
        ],
      });
      service = TestBed.inject(InventoryPickService);
    });
    ...
    it('resolvePickScan propagates a facade failure', () => {
      facade.resolvePickScan.mockReturnValue(throwError(() => new Error('not found')));
    ...
    it('confirmPickLine calls the facade with workorderId, pickTaskId, pickLineId (= pickTaskId), and quantityPicked', () => {
      facade.confirmPickLine.mockReturnValue(of(taskFixture));

      let result: unknown;
      service.confirmPickLine('wo-001', 'task-001', 3).subscribe(value => (result = value));

      expect(facade.confirmPickLine).toHaveBeenCalledExactlyOnceWith('wo-001', 'task-001', 'task-001', {
        quantityPicked: 3,
      });
      expect(result).toEqual({
        pickTaskId: 'task-001',
    ...
    it('completePickTask calls the facade with workorderId, pickTaskId, and an empty body when no reason is given', () => {
      facade.completePickTask.mockReturnValue(of(taskFixture));

      service.completePickTask('wo-001', 'task-001').subscribe();

      expect(facade.completePickTask).toHaveBeenCalledExactlyOnceWith('wo-001', 'task-001', {});
    });

    it('completePickTask forwards an explicit reason', () => {
      facade.completePickTask.mockReturnValue(of(taskFixture));

      service.completePickTask('wo-001', 'task-001', 'short pick').subscribe();
    ...
    it('getPickTasks calls the facade with workorderId and maps the response array to PickTaskLine[]', () => {
      facade.getPickTasks.mockReturnValue(of([taskFixture]));

      let result: unknown;
      service.getPickTasks('wo-001').subscribe(value => (result = value));

      expect(facade.getPickTasks).toHaveBeenCalledExactlyOnceWith('wo-001');
      expect(result).toEqual([
        {
          pickTaskId: 'task-001',
          productSku: 'SKU-001',
