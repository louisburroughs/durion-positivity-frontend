/**
 * Domain shapes for the live supplier stock-availability check used by the
 * purchase-order line check (#212).
 *
 * Mirrors `@durion-sdk/supplier`'s `SupplierStockAvailability` response, kept
 * separate from the generated SDK types per this repo's convention. Kept as
 * its own copy in this feature (rather than imported from `product`) so
 * `inventory` does not reach across a feature boundary for it.
 *
