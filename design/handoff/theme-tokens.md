...
Both faces are self-hosted under `src/assets/fonts/` (`barlow/`, `noto-sans/`).

### Spacing & radius

| Token | Value |
|---|---|
| `--space-1 / 2 / 3 / 4 / 6 / 8` | 0.25 / 0.5 / 0.75 / 1 / 1.5 / 2 rem |
| `--radius-sm / md / lg` | 4 / 8 / 16 px |

> There is **no `--space-5`**. Do not reference it.
...
| `--space-5` | not declared — pick `--space-4` or `--space-6` |
